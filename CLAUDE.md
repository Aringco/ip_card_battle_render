...
**패널에는 옛 LCD 같은 가로 주사선이 얹혀 있다.** `.lobby-panel::after`의 `repeating-linear-gradient`다. `.lobby-panel-bg`가 아니라 **버튼 자신**에 거는 이유 두 가지 — 배경 레이어에는 `blur`/`scale` filter가 걸려 있어 그 안에 그리면 줄무늬까지 뭉개지고, `::after`는 마지막 자식이라 그림과 글자 **둘 다** 위에 얹힌다(실제 화면이라면 글자에도 주사선이 지나가는 것이 맞다).
