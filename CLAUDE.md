...
**폼 카드는 넓은 화면에서 `zoom`으로 통째로 키운다.** 스테이지가 738×590인데 접힌 카드가 358×326이라 절반이 비어 있었다 — `--form-zoom`으로 액자·귀퉁이 장식·글자·입력창을 한 번에 확대한다. `transform: scale()`이 아니라 `zoom`인 이유는 scale이 레이아웃 상자를 그대로 둬서 스테이지 밖으로 삐져나가고 `measureLobby` 실측이 눈에 보이는 것과 어긋나기 때문이다. **규칙을 펼치면 1로 되돌린다** — 그때는 카드가 이미 스테이지 폭을 다 쓰고 있어 더 키울 자리가 없고, 렌더 폭이 그대로인 채 글자만 커지면 규칙 라벨이 다시 말줄임으로 잘린다. 배율은 높이로 나눈다: `≥768px` 1.45, `≥768px && ≥1000px` 1.75 — 1280×900에서 1.75를 쓰면 가장 키가 큰 폼(초대 링크로 들어온 방 참가하기)이 645px가 되어 590px를 넘긴다(실측 상한 약 1.50).
