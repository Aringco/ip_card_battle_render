...
**게임 규칙을 펼치면 폼이 세로로 자란다.** 한때는 오른쪽으로 넓어지는 2열이었지만(`51d6d24`~`8efe72e`) 2026-09-08에 세로로 되돌렸다. 이 로비에서 늘 모자란 것은 세로이므로 **되돌린 대가를 두 곳에서 치른다**:

1. `--form-zoom`을 **1로 내린다.** 확대까지 걸린 채로 세로로 자라면 스테이지 박스를 그대로 넘긴다.
2. 제출 버튼(통나무집 그림)을 **76 → 40px로 줄인다.** 1로 내려도 1280×900에서 카드가 583px가 되는데 스테이지가 590px이라 여백까지 더하면 넘쳤다(실측 3건). 40px이 그 36px을 정확히 메우고, 그 값은 좁은 화면에서 이미 쓰던 값이자 걷어낸 초록 막대와 같은 키라 **더 줄일 자리가 없다.**

상태는 `GameRulesFields`가 `data-rules-open` 속성으로 알리고 CSS가 `:has()`로 읽으므로, 폼 3종에 prop이 늘지 않는다. **규칙 항목을 하나라도 더하면 이 균형이 깨진다 — `measureLobby.mjs`로 반드시 다시 잴 것.**
