...
로비 실측 기준값 — **폼에 항목을 추가했다면 이 값과 대조하세요.** (2026-09-08 기준 — 규칙 펼침이 **카드 안쪽 스크롤**로 바뀐 뒤)

| 뷰포트 | 안전영역 | 싱글+규칙 | 방 만들기+규칙 | 방 만들기+충돌 | 방 만들기+관전 | 참가+초대 |
| --- | --- | --- | --- | --- | --- | --- |
| 1280×900 | 738×628 | 484×558 | 484×558 | 484×558 | 484×558 | 484×521 |
| 1920×1080 | 885×754 | 573×683 | 573×683 | 573×683 | 573×683 | 573×616 |
| 390×844 | 371×589 | 358×538 | 358×538 | 358×538 | 358×538 | 358×374 |
...
> 다시 재야 합니다.**
>
> ⚠️ **규칙을 펼치면 카드가 자라지 않고 "스테이지 높이에서 멈춘 뒤 안쪽이 스크롤"합니다.**
> 그래서 펼친 세 줄이 뷰포트마다 **같은 값**(558 / 683 / 538)으로 고정돼 있습니다 —
> 그 값이 곧 그 화면에서 카드가 가질 수 있는 최대 높이입니다.
>
> 2026-09-08 오후까지는 카드가 세로로 자라고 `.stage-form`이 통째로 스크롤했는데(849·1003·621),
> 그러면 **나무 액자까지 함께 밀려 올라가** 판이 화면 밖으로 나갔습니다. 지금은
> `.lobby-form-board`가 `max-height: 100%`로 멈추고 `.lobby-form-scroll`이 안에서 스크롤합니다.
> `measureLobby.mjs`는 그 라벨에서 폼 스크롤을 `yes(정상)`으로 표시합니다.
>
...
**그리고 규칙을 펼쳤을 때 카드가 자라지 않습니다.** `.lobby-form-board`가 `max-height: 100%`로
스테이지 높이에서 멈추고, 새로 만든 `.lobby-form-scroll`이 그 안에서 스크롤합니다. 제출 버튼은
스크롤 밖에 둬서 규칙을 끝까지 내리지 않아도 늘 눌립니다.

> ⚠️ **`max-height`를 `--form-zoom`으로 나누지 마세요.** 요즘 Chrome의 `zoom`은 퍼센트를
> 이미 확대분만큼 보정해 해석합니다 — 한 번 더 나눴더니 카드가 배율만큼(1.35배) 작아져,
