# 다른 PC에서 이어서 작업하기 — 2026-09-07 기준 (결과 화면 2열 + UI 아이콘까지)
...
| — | `오늘의_명령어_*.txt` | 날짜별로 무엇을 왜 요청했고 무엇이 바뀌었는지 (최신: `2026-09-07`) |

> `ROADMAP.md`는 초기 설계 문서로 실제 코드와 많이 다릅니다. 참고만 하세요.

> ✅ **upstream 재조정은 2026-09-05 기준 최신입니다.** 두 차례(7커밋 + 12커밋) 병합해
> **upstream 미반영 커밋 0건**입니다. 두 번째 병합으로 **관전자(제3의 자리)**, 시작 공유
> 카드 2장, 결과 화면 개편 등이 들어왔습니다 — §5-0 참조.
>
...
| 하 | **`laurel_book.webp`가 미아** | `icon_rank`로 대체돼(2026-09-07) **어디서도 쓰이지 않는데** 프리로드 목록에는 계속 들어갑니다. 되돌릴 일이 없다면 지우세요 |
...
- **배포** — `render.yaml`(Blueprint)까지 준비돼 있습니다. Render 대시보드에서
  **New + → Blueprint → 이 저장소**를 한 번 고르면 서비스가 만들어지고, 그 뒤로는
  `main`에 push할 때마다 자동 재배포됩니다. **넣어야 할 환경변수는 없습니다.**
