...
로비 실측 기준값 — **폼에 항목을 추가했다면 이 값과 대조하세요.** (2026-09-08 기준 — 규칙 펼침이 스크롤로 바뀐 뒤)

| 뷰포트 | 안전영역 | 싱글+규칙 | 방 만들기+규칙 | 방 만들기+충돌 | 방 만들기+관전 | 참가+초대 |
| --- | --- | --- | --- | --- | --- | --- |
| 1280×900 | 738×628 | 484×782 | 484×849 | 484×849 | 484×849 | 484×521 |
| 1920×1080 | 885×754 | 573×924 | 573×1003 | 573×1003 | 573×1003 | 573×616 |
| 390×844 | 371×589 | 358×572 | 358×621 | 358×621 | 358×621 | 358×374 |
...
> 다시 재야 합니다.**
>
> ⚠️ **규칙을 펼친 세 줄(`+rules`/`+clash`/`+spectator`)은 이제 스테이지를 넘깁니다 —
> 그리고 그게 정상입니다.** 2026-09-08에 "펼치면 카드 크기를 그대로 두고 넘치는 만큼
> 스크롤한다"로 바꿨기 때문입니다. `measureLobby.mjs`도 그 라벨에서는 폼 스크롤을 문제로
> 세지 않고 `yes(정상)`으로 표시합니다. **이 표의 그 값들은 "넘치면 안 되는 상한"이 아니라
> 그저 기록입니다.**
>
