...
| 자리 | 그림 | 규칙 |
| --- | --- | --- |
| 승패 표시 | 🥇 `medal_1` / 🥈 `medal_2` | 동그란 팀 마크(🟢/🔵)를 대체. 내가 이겼거나 **관전자**면 금, 졌으면 은. **무승부는 🤝 이모지 그대로** |
| 두 판의 윗변 | 🌿 `laurel_book` | `.board-crest` — `translate(-50%, -50%)`로 윗변을 정확히 반씩 물고 앉는다 |
...
> 메달은 **폭이 아니라 높이를 맞춥니다**(`h-24`). 금메달이 밀 이삭 때문에 훨씬 넓어서,
> 폭을 맞추면 은메달만 커 보입니다.
