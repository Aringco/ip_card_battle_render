...
| 자리 | 그림 | 규칙 |
| --- | --- | --- |
| 승패 표시 | 🥇 `medal_1` / 🥈 `medal_2` | 높이 `h-36`(144px). 동그란 팀 마크(🟢/🔵)를 대체. 내가 이겼거나 **관전자**면 금, 졌으면 은. **무승부는 🤝 이모지 그대로** |
| 두 판의 윗변 | 🌿 `laurel_book` | `.board-crest` — 폭 `--crest-w: 53px`. `translate(-50%, -50%)`로 윗변을 정확히 반씩(위 22px / 아래 22px) 물고 앉는다 |
...
> ⚠️ **문장을 얹은 판은 위쪽에 자리를 스스로 낸다.** 위로 삐져나온 절반만큼 바로 위 요소와
> 겹치기 때문입니다 — `.board-frame:has(> .board-crest)`의 `margin-top`이 그 몫이고,
> 계수 `0.42`는 **그림 비율(320:268)의 절반**입니다. 다른 비율의 그림으로 갈면 이 값도
> 함께 고쳐야 겹침이 다시 생기지 않습니다.

> 메달은 **폭이 아니라 높이를 맞춥니다**(`h-36`). 금메달이 밀 이삭 때문에 훨씬 넓어서,
> 폭을 맞추면 은메달만 커 보입니다. 대신 `h-36`에서 금메달 폭이 321px이라 390px 화면의
> 콘텐츠 폭(326px)에 겨우 들어가므로 `max-w-full object-contain`을 방어선으로 붙였습니다.
