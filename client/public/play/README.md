...
`upper_bar`는 **왼쪽 마구리가 곧 팻말**이라 두께가 `--beam-h`의 2.2배(1600×900에서
240px)나 됩니다. 이걸 헤더 **자신의** `border-left`로 그리면, 절대배치 자식의 기준
상자가 부모의 **padding box**라 팻말 글자가 통째로 240px 오른쪽 나무 위에 얹힙니다
(좌표를 정확히 계산했는데도 그렇게 나왔습니다 — `left`는 58.86px로 맞았고 실제
렌더는 299px이었습니다).

`::before`(`inset: 0`)로 옮기면 헤더는 테두리 없는 상자로 남아 padding box =
border box가 되고 좌표가 그대로 맞습니다. z-index는 주지 않습니다 — 의사요소는 첫
자식이라 뒤따르는 글자·알약이 저절로 위에 옵니다.

### ⚠️ 치수는 `--beam-h` 하나에서 파생시킨다

상단 바 높이는 `clamp(92px, 12vh, 124px)`로 화면을 따라 바뀝니다. 마구리 폭과
팻말 좌표를 고정 px로 두면 높이만 변할 때 **팻말만 홀로 찌그러집니다.** 그래서
전부 `calc(var(--beam-h) * n / 189)` 꼴로 씁니다(189 = 잘라낸 그림의 높이).
실측값 네 벌 — 왼쪽 마구리 420 · 오른쪽 150 · 팻말 x 103~339 · 팻말 y 57~140.
**그림을 다시 자르면 이 네 벌을 함께 고칠 것.**
