...
// ⚠️ **범위를 좁혔다.** 예전 [0.85 … 1.55]는 기본(1.12) 대비 0.76~1.38배라, 큰 쪽
// 몇 단계에서 화면이 무너졌다 — 플레이 화면은 대부분 액자 크기에 비례(vh·%)해 글자만
// 1.4배가 되면 알약·카드·타이머 창을 넘쳐 나온다. 반대로 작은 쪽은 액자는 그대로인데
// 글자만 줄어 빈 자리가 크게 뜬다.
// 이제 기본 대비 **0.84~1.16배**만 움직인다(단계 간 0.06씩 고르게).
export const FONT_SCALE_STEPS = [0.94, 1, 1.06, 1.12, 1.18, 1.24, 1.3] as const;
// 1~7 단계. 기본은 **4단계(1.12배)** — 이 기능이 생기기 전의 크기 그대로다.
