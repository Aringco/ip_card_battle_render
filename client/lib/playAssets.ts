...
export const PLAY_ASSETS = {
  ...
  /**
   * 플레이 화면 **통짜 액자**(1965×1000) — 칸막이까지 그려진 한 장.
   *
   * 2026-09-18에 새 그림(밧줄 모서리·이끼)으로 갈았다. 원본 `assets-src/play_frame.png`를
   * **그대로** 자른다 — 좌우 차이가 1~2px뿐이라 `scripts/mirrorPlayFrame.mjs`(왼쪽 절반
   * 뒤집기)는 돌리지 않았다. 돌리면 오른쪽에만 있는 덩굴이 사라진다.
   *
